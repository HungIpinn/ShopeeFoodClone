  ...
  return (
    <SafeAreaView style={{ flex: 1 }}>
      <View style={styles.container}>
        ...
        <MyTextInput title="Họ tên" />
        <MyTextInput title="Email" />
        <MyTextInput secureTextEntry={true} title="Password" />
        <MyButton
          PressableStyle={styles.btnRegister}
          title="Đăng ký"
          PressableTxtStyle={styles.txtResiger}
          onPress={() =>
            dispatch(loginUser({ username: 'hung', password: '123' }))
          }
        />
        <MyButton
          PressableStyle={styles.btnRegister}
          ...
          PressableTxtStyle={styles.txtResiger}
          onPress={() =>
            dispatch(TestJson({ username: 'foo', password: 'bar', id: 1 }))
          }
        />
        <SocailMeidaButton TextInLine="Đăng ký với" />
      </View>
    </SafeAreaView>
  );
};
